...
/**
 * Copia del archivo /course/modedit que permite adicionar ó actualizar módulos en un curso
 *
 * Este archivo ha sido modificado para tener total control sobre la creación de módulos desde el Módulo de Actividad Kitmoo
 * ya que permite realizar nuevas acciones (redirecciones, acceso a la Base de Datos, etc.) para que todo funcione correctamente.
 *
 * @package    mod
 * @subpackage kitmoo
...
require_once($CFG->dirroot . '/mod/kitmoo/locallib.php');    // Cambio 1: Agregamos locallib.php del Módulo de Actividad Kitmoo.
...
$k  = optional_param('k', 0, PARAM_INT);    // Cambio 2: Obtenemos parámetro - Id de la instancia del Módulo de Actividad Kitmoo.

$url = new moodle_url('/course/kitmoo_modedit.php');    // Cambio 3: Establecemos la URL correcta.
if (!empty($return)) {
    $url->param('return', $return);
}

$url->param('k', $k);    // Cambio 4: Agregamos parámetro - Id de la instancia del Módulo de Actividad Kitmoo.
...
$PAGE->set_pagelayout('base');    // Cambio 5: Establecemos el layout de la página a "base".
...
$mform->moodleform('kitmoo_modedit.php?k='.$k);    // Cambio 6: Configuramos el action del formulario.
$mform->set_data($data);

if ($mform->is_cancelled()) {
    if ($return && !empty($cm->id)) {
        redirect("$CFG->wwwroot/mod/kitmoo/create.php?id=$k");    // Cambio 7: Redireccionamos a /mod/kitmoo/create.php.
    } else {
        redirect("$CFG->wwwroot/mod/kitmoo/create.php?id=$k");    // Cambio 8: Redireccionamos a /mod/kitmoo/create.php.
    }
    ...
    // Cambio 9: Guardamos los íconos en la Base de Datos.
    $record = new stdClass();
    $record->source = 'pix/' . $module->name . '.png';
    $record->type = $module->name;
    $record->url = "$CFG->wwwroot/mod/$module->name/view.php?id=$fromform->coursemodule";
    $record->positiontop = rand(10, 400) . 'px';
    $record->positionleft = rand(10, 400) . 'px';
    kitmoo_insert_icon($k, $record);

    if (isset($fromform->submitbutton)) {
        if (empty($showgradingmanagement)) {
            redirect("$CFG->wwwroot/mod/kitmoo/create.php?id=$k");    // Cambio 10: Redireccionamos a /mod/kitmoo/create.php.
        } else {
            $returnurl = new moodle_url('/mod/kitmoo/create.php', array('id' => $k));    // Cambio 11: Redireccionamos a /mod/kitmoo/create.php.
            redirect($gradingman->get_management_url($returnurl));
        }
    } else {
        redirect("$CFG->wwwroot/mod/kitmoo/create.php?id=$k");    // Cambio 12: Redireccionamos a /mod/kitmoo/create.php.
    }
    exit;
